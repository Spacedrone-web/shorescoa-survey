...
async function smtpSend(to: string, subject: string, body: string): Promise<void> {
  ...
  async function readLine(): Promise<string> {
    while (true) {
      const i = buf.indexOf("\n");
      if (i !== -1) { const l = buf.slice(0,i).replace(/\r$/,""); buf = buf.slice(i+1); return l; }
  ...
  async function readResp(): Promise<string> {
    let last = "";
    while (true) { last = await readLine(); if (last.length < 4 || last[3] !== "-") break; }
    return last;
  }
  ...
  try {
    await readResp();
    await cmd("EHLO shorescoa.com");
    await cmd("AUTH LOGIN");
    await cmd(btoa(SMTP_USER));
    const ar = await cmd(btoa(SMTP_PASS));
...
export async function POST({ request, cookies }: APIContext) {
  ...
  if (cookies.get("admin_auth")?.value !== "shores-admin-ok") return j({ ok: false, error: "Unauthorized" }, 401);
  try {
    const { to, guestName, isTest } = (await request.json()) as { to: string; guestName?: string; isTest?: boolean };
    ...
    const body    = (isTest ? "[TEST EMAIL \u2014 PLEASE IGNORE]\n\n" : "")
